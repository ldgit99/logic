...
export async function sendAssessment(assessmentData) {
  const result = await postWithFallback('/assessment', assessmentData);
  if (!result.ok) throw new Error('assessment 전송 실패 (모든 Worker URL 응답 없음)');
...
async function postWithFallback(path, body) {
  const result = await tryPost(path, body);
  if (!result.ok) {
...
async function tryPost(path, body) {
    ...
    try {
      ...
      if (res.ok || res.status === 201 || res.status === 200) {
        ...
        if (firstPayload === null) {
          ...
          firstPayload = text ? JSON.parse(text) : null;
        }
      }
    } catch {
      // ??쇱벉 URL ??뺣즲
  ...
  return { ok: anySuccess, data: firstPayload };
}
...
async function flushQueue() {
  ...
  for (const item of queue) {
    ...
    const result = await tryPost(item.path, item.body);
    if (result.ok) continue; // ?源껊궗: ?癒?퓠????볤탢
